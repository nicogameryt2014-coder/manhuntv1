...
export function step(
  ...
  const jugador = st.entities.find((e) => e.isPlayer)!;
  aplicarInput(st, jugador, input, dt);

  // jugadores conectados por internet: mueven su propia entidad, sin IA
  if (remotos) {
    for (const e of st.entities) {
      const inp = remotos[e.id];
      if (inp) aplicarInput(st, e, inp, dt);
    }
  }
  ...
  for (const e of st.entities) {
    if (!e.vivo || e.isPlayer || (remotos && remotos[e.id])) continue;
